...
**Canonicalization (current state, v1).** Two serialization rules coexist and a second implementer must use the right one per layer. *Measurement layer* (`WitnessReceipt.receipt_hash`, `verify_receipt_integrity`): SHA-256 over `json.dumps(obj, sort_keys=True)` — Python's default **spaced** separators (`", "`, `": "`). *Deed layer* (`ActionReceipt` hashes, certificates): SHA-256 over `json.dumps(obj, sort_keys=True, separators=(",", ":"))` — **compact**. Unification into one rule (`CANON_VERSION` 2, RFC 8785/JCS candidate) is deferred to the v0.2 release decision so canon migrates exactly once (`spec/action-receipt-v0.2-draft.md`, `spec/ADR-001-standing-model.md`).
