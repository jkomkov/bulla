...
**Canonicalization (current state, `CANON_VERSION` 2).** This paragraph previously described the two-rule split as current and the unification as deferred to the v0.2 release decision. That decision was taken with the normative v0.2 wire release on 2026-07-13, and the unification shipped. One rule now governs every minting site: `bulla._canonical.canonical_json` — SHA-256 over `json.dumps(obj, sort_keys=True, separators=(",", ":"))`, compact, key-sorted, `\uXXXX`-escaped non-ASCII. The measurement layer's former **spaced** form survives only as `legacy_json_v1`, a verification fallback that must never mint; deed-layer hashes were already compact and are byte-unchanged. See §"Canonicalization (normative — `CANON_VERSION` 2)" above, which governs, and `spec/action-receipt-v0.2.md` for the two deliberate RFC 8785 deviations.

One deliberate exception remains, and it is not a second canon rule: `Composition.canonical_hash()` and `Diagnostic.content_hash()` are *input identities* pinned inside existing receipts and retain their original spaced serialization, because a verifier compares those stored values rather than recomputing them. Migrating them is a `CANON_VERSION`-3 decision, dated open (2026-07-13).
...
**Constitutional mapping — the normative half.** The five fields the [constitutional design brief](https://github.com/jkomkov/res-agentica/blob/main/abridged/constitutional-proposal-design-brief.md) names (§II, Receipt Architecture) map onto this envelope one-to-one: Act = the certified content scope · Authority = `authority` (delegation chain → surviving principal + `policy@hash`) · Bounds = `bounds` · Justification = the recomputable diagnostic itself · Appeal Path = `recourse`. (Also abridged Appendix C.) The exact Articles in the constitutional proposal govern; this envelope is the running reference implementation. Civic asymmetry is a schema invariant from day one — `retention_class ∈ {authority-permanent, operational, personal-expiring}` — while any forgetting mechanism is a later, separately pre-registered instrument.
