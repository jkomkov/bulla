...
**Constitutional mapping — the normative half.** The five fields the [constitutional design brief](https://github.com/jkomkov/res-agentica/blob/main/essays/constitutional-machine-design-brief.md) names (§Receipt — the normative source) map onto this envelope one-to-one: Act = the certified content scope · Authority = `authority` (delegation chain → surviving principal + `policy@hash`) · Bounds = `bounds` · Justification = the recomputable diagnostic itself · Appeal Path = `recourse`. (Also abridged Appendix C.) Where the two conflict the brief's Orientation governs; this envelope is the running reference implementation. The civic asymmetry is a schema invariant from day one — `retention_class ∈ {authority-permanent, operational, personal-expiring}` — while the forgetting *mechanism* is a later, separately pre-registered instrument.
