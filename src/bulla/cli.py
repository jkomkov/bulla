...
def _seed_set_compositions(repo_root: Path) -> list[tuple[Composition, str]]:
    ...
    nested_bulla = repo_root / "bulla"
    bulla_root = (
        nested_bulla
        if (nested_bulla / "pyproject.toml").is_file()
        and (nested_bulla / "src" / "bulla").is_dir()
        else repo_root
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "Use `receipt create` / `receipt verify` to record and check action "
            "integrity, authority, bounds, evidence, reliance, and recourse. "
            "Verification reports what is proven and what remains unresolved; it "
            "does not establish worldly truth. Legacy composition diagnostics "
            "(`compose` / `audit` / `gauge`) report seam blind spots and the "
            "model-relative coherence fee as disclosure and omission signals."
        ),
    )
    parser.add_argument(
