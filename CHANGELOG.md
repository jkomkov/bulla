...
capability with an ASCII-safe receipt-first help line. The 0.47.0 candidate
was not uploaded to PyPI; its Windows release-smoke gate rejected a Unicode
arrow under the default console encoding. The public slot and tag remain as
...
## 0.47.0 — 2026-08-04 (not published)
...
release ceremony without prematurely standardizing a witness network.

### Release truth and self-coverage
- PyPI is now the primary release anchor. `bulla coverage` reconciles the
  package's actual published versions and accepted artifact digests, verifies
  each candidate receipt before counting it, and reports `contemporaneous`,
  `reconstructed`, `missing`, and `invalid` separately. Git remains a secondary
  anchor and now rejects candidates, experiments, and other non-package tag
  pollution.
- The publication workflow now publishes first, verifies the accepted wheel and
  sdist through PyPI's Integrity API, then mints and signs the release receipt
  and attaches it to a durable GitHub release. A candidate can never become a
  contemporaneous receipt by relabeling.
- The historical 0.37.0 and 0.40.0 receipts remain reconstructions. The 0.43.0
  local artifact remains a release candidate and never counts as published
  coverage.

### Authority binding (v0.3 draft; envelope-swap fix)
- Closes an authority-forgery gap. `content_hash` is envelope-free by design, so
  a signature over content authenticated the *claim* but not the mandate/remedy:
  an attacker could keep a valid content signature, swap the
  `authority`/`bounds`/`recourse` envelope, recompute the two unsigned downstream
  hashes, and present forged authority that still verified to `attestation`.
- Adds the draft ActionReceipt v0.3 wire extension,
  `ActionReceipt.authorization_hash = H(content_hash, envelope_hash)`, and an
  optional `authorization` proof over it (`sign_action_receipt(receipt, signer)`
  produces both proofs). `verify_receipt` now reports **content authenticity**
  (`checks['signature']`) and **authority authenticity**
  (`authority_authentic ∈ {verified, forged, unauthenticated, unresolved,
  not_applicable}`) as separate verdicts; a swapped envelope under a valid
  content signature fails as `forged`.
- Versioned compatibility: v0.1/v0.2 receipts and preimages remain byte-identical
  under the new verifier. Authorization-bearing receipts use
  `schema_version: "0.3"`; an old v0.2-only verifier is not expected to accept
  them. `sign_action_receipt` upgrades v0.2 to v0.3 before signing. A receipt
  signed over content only is reported `authority_authentic='unauthenticated'`,
  never silently treated as vouched.
- Both proofs must name the same issuer and verification method. This rejects a
  signer-substitution attack in which an attacker retains the honest content
  signature but re-signs the swapped envelope with the attacker's own key.
- Bound on the claim: `authority_authentic='verified'` proves that the content
  signer signed the exact envelope. The structured did:key delegation profile
  below separately validates the bounded chain claim; semantic policy obedience
  and external principal resolution remain open.
- New adversarial vector/tests exercise the full attack: swap the mandate,
  recompute every unsigned hash, retain the original signature — must fail
  authorization (`tests/test_action_receipt.py`).

### Delegation profile (v0.3 opt-in released draft)
- Adds signed, domain-separated did:key grants and reports six independent
  dimensions: chain, principal, policy digest, exact receipt scope, typed time,
  and revocation. They remain surfaced rather than folded into receipt integrity.
- Closes four hostile-review findings: scope is bound to this receipt rather than
  merely equal across grants; every upstream key derives from its own grantor and
  never from a receipt-level key override; unknown grant/proof fields fail closed;
  and unresolved revocation can no longer satisfy the strong reliance predicate.
- `cryptographically_bound` names the bounded offline claim. `fully_delegated`
  additionally requires a typed checkpoint within every closed grant window and
  positive non-revocation evidence; it is false while revocation transport is
  unbuilt.

### Reliance — the consumer half of answerability (`bulla.reliance`)
- New `bulla.reliance`: a **`ReliancePolicy`** (named, versioned, hashable, modelled on
  `model.PolicyProfile`) declares the dimension values a relying party will accept, and
  a pure, crypto-free **`decide(verification, policy) → RELY | REFUSE | ESCALATE`**
  combines the surfaced dimensions into one honest decision. Rejections are routed: a
  CLEAR violation (`forged`, `violates`, `broken`, `mismatch`) forces `REFUSE`, while an
  INCONCLUSIVE state (`unresolved`, `unauthenticated`, `not_checkable`) routes to
  `ESCALATE` — forum-completeness on the consumer side. `ReceiptVerification.to_dict()`
  makes the decision surface serializable/pinnable.
- The unbuilt-revocation gap becomes a **declared, recorded choice**: there is no
  implicit policy. `STRICT_RELIANCE_POLICY` refuses `unresolved` revocation, while
  a caller that selects `PRAGMATIC_RELIANCE_POLICY` explicitly accepts that risk —
  and the selected policy is then on the record.
- **Receipted reliance.** `build_reliance_receipt` mints a **`bulla.rely`** receipt — an
  ordinary `ActionReceipt` (an `action.type`, NOT a new type, per THE ONE ABSTRACTION),
  which verifies under the existing `verify_receipt`. The relier signs its own envelope
  and is thereby answerable for its reliance (reflexivity). `verify_reliance` recomputes
  `decide(verify_receipt(relied_on), policy)` and checks the claim, so **fault becomes a
  calculation**: a lying relier (claims RELY where the policy says REFUSE) is caught by
  the same recomputation that catches a lying actor. Golden vector `reliance-rely.json`
  is validated by the zero-import checker (15/15). All four verdict objects and the new
  `RelianceDecision` raise on `bool()` (the R0 discipline).
- Reliance verification now returns a typed `RelianceVerification`, whose boolean
  use also raises. It authenticates the `bulla.rely` receipt and its authority,
  requires a complete closed verification view, recomputes the pinned decision,
  and binds the relied-on receipt by both `event` and `attestation` hashes.
  Malformed policy values fail at construction.

### Routed-inference profile (v0.1 draft; specified, local traces only)
- Adds the Answerability Conservation Law: no consequential route edge may create
  an orphaned consequence or silently discharge an inherited binding. The draft
  now fails closed on every attempted discharge; a bare closure or novation
  reference has no effect. Delivery and reliance are explicitly not closure.
- Narrows the executable grammar to one orderer, one router, one accepting
  provider, one delivery assertion, and one relier. Terms declare
  `route_topology=single_route_single_provider`, `term_disclosure=full`, and
  route depth 1. Selective disclosure, attenuation, multi-hop routing, and DAG
  accounting remain deferred rather than implied.
- Separates signed recourse conveyance from operational reachability. The local
  corpus can verify inherited recourse terms but reports reachability unverified.
- Adds a zero-import profile checker and fourteen single-purpose adversarial traces,
  plus a machine-readable finite violation taxonomy and resign-and-mutate tests.
- Adds the disclosed-components integer ledger with separate
  `ACCOUNTING_CONFORMS` and `SETTLEMENT_CONFORMS` depths. Fixture receipts are
  measured in a generated size report; those numbers are not protocol limits.
- Adds an isolated offline handoff demo and deterministic external reproduction
  bundle. The supplied checker reproduces fixtures; it does not count as an
  independent implementation. Live provider, settlement, external-reproduction,
  and independent-witness counts remain zero.
- Cross-implementation vectors now include malformed executable-form rejection;
  the Python verifier, zero-import checker, and Glyph TypeScript verifier align on
  v0.3 preimages, authorization presence, containers, and portable-scope handling.

### `bounds_conformance` — the missing half of authorization (v0.3 draft)
- `bounds.scope` may now be EITHER prose (a string, unchanged) OR a structured
  `jsonschema+quantum/1` predicate over the act's `action.subject` (a dict, under
  `deed_schema: "0.3"`). When it is structured, `verify_receipt` recomputes a new
  **crypto-free, digest-rung** dimension `bounds_conformance ∈ {conforms, violates,
  not_checkable, not_applicable}` — *did the act obey its declared scope*. Combined
  with the existing `scope_binding` (*did the chain convey that scope*), this closes
  real authorization: conveyed **and** obeyed. Neither needs the deferred attenuation
  lattice, because the chain requires scope-digest equality, so narrowing cannot occur.
- Surfaced, never folded: an act that exceeds its scope still produces a valid record
  (`ok=True`, authority `verified`, `scope_binding=verified`); only `bounds_conformance`
  reports `violates`, for a relying party to act on.
- Reuse, not a new language: the `jsonschema+quantum/1` form, its validator, and its
  evaluator now live in a shared leaf module `bulla.executable_form`, used by both
  conventions and scope. A prose scope hashes **byte-identically** to before
  (`definition_hash` is str→UTF-8, dict→canonical JSON), so the entire existing vector
  corpus is unchanged. Golden vectors `delegated-scope-{conforms,violates}.json` are
  reproduced by the zero-import checker (now 13/13). The Glyph TypeScript verifier
  mirrors `boundsConformance` at the digest rung.

### Verdict objects have no boolean truth value (footgun closed)
- **`ReceiptVerification`, `Authenticity`, `DelegationVerdict`, and `GateDecision`
  now raise `TypeError` on `bool()`.** Each is a plain object and was therefore
  unconditionally truthy, so `if verify_receipt(d): ...` was *always* taken — it
  never consulted `.ok` and silently accepted receipts that had FAILED. A verdict
  has no single truth value (a record can be authentic yet unauthorized, hash-valid
  yet unsigned), so the ambiguous read is now a loud error, numpy-style. Read `.ok`
  for record integrity, the named dimensions for what each proves, or (once built)
  `bulla.reliance.decide(...)` for a rely/refuse answer. No deprecation cycle: no
  correct program depended on the old behaviour, and a filtered `DeprecationWarning`
  would let the silent always-accept continue for a release.
- Housekeeping surfaced alongside: `GateDecision.refusal_certificate` (a declared
  field never assigned — refusal certificates are built separately) was removed, and
  `ADVISORY_GATE_POLICY` is now a literal alias of `DEFAULT_GATE_POLICY` (they were
  already field-for-field identical).

### Recourse truth
- Fee gating remains explicit opt-in via `--require-fee`; the default gate does
  not block on the coherence fee. A regression test now pins that the default
  policy PROCEEDs on a positive fee (reported, never gated), so the retired
  detection thesis cannot re-enter the enforcement path as a default.
- The cross-boundary bond demo is retired. It incorrectly treated the fee as an
  objective slash predicate and collateral cap. Collateral is an external
  policy or settlement sidecar; `ActionReceipt.stake` remains `null`.

### Experimental equivocation evidence
- Adds exactly one witness-adjacent artifact,
  `bulla.experimental.EquivocationEvidence`, and one checker. It establishes
  equivocation only for authentic same-operator, same-log, same-size heads with
  different roots. It defines no witness bundle, HTTP protocol, pool, bond, or
  operator market.

### Erratum (0.43.0)
The 0.43.0 entry said its workflow minted the package receipt "at release time"
and closed the retroactive seam. The workflow in fact minted before PyPI
accepted the distribution and uploaded only an ephemeral Actions artifact.
That artifact was a release candidate, not a contemporaneous published-release
receipt. The corrected post-publication ceremony ships in 0.44.0; history is
...
- **[ERRATUM 2026-07-12 — the canonicalization item below did NOT ship in 0.42.0.]** The struck entry described a unification that was planned but is not in the 0.42.0 tree: there is no `bulla._canonical.canonical_json`, no `tests/test_canonicalization.py`, no `CANON_VERSION` constant, and no "Canonicalization" section in `WITNESS-CONTRACT.md`. **Actual state of canon in 0.42.0 — a two-rule split:** the *deed layer* (`action_receipt.py`, `certificate.py`) hashes over `json.dumps(obj, sort_keys=True, separators=(",", ":"))` (compact); the *measurement layer* (`witness.py` `WitnessReceipt`) hashes over `json.dumps(obj, sort_keys=True)` (default **spaced** separators). A second implementer reproducing a `WitnessReceipt` hash must use the spaced form. The unification is deliberately deferred to v0.2, together with the RFC 8785/JCS decision and the `CANON_VERSION`-2 reslice (`spec/action-receipt-v0.2-draft.md`, `spec/ADR-001-standing-model.md`), so canon migrates exactly once. Original entry retained, struck, for the audit trail:
